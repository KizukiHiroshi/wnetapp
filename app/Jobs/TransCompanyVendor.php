...
class TransCompanyVendor implements ShouldQueue
{
    ...
    private function updateNewTable($untreatedrows, $newtablename) {
        foreach ($untreatedrows as $untreatedrow) {
            ...
            $form['pic'] = $untreatedrow->先方担当者名;
            $form['telno'] = $untreatedrow->電話番号;
            $form['faxno'] = $untreatedrow->FAX番号;
            $form['emails'] = $untreatedrow->MailAdd;
            $form['orderdayofweek'] = $untreatedrow->発注曜日;
            $form['arrivaldayofweek'] = $untreatedrow->入荷曜日;
            $form['freeshippingquantity'] = $untreatedrow->無料入荷数量;
            $form['freeshippingamount'] = $untreatedrow->無料入荷下代;
            $form['price_rounding_opt'] = $untreatedrow->下代端数処理;
            $form['is_cansenddirect'] = $untreatedrow->直送可;
            $form['shippinggremarks'] = $untreatedrow->出荷条件;
            $form['closingdate_opt'] = $untreatedrow->締日;
            $form['tax_rounding_opt'] = $untreatedrow->消費税処理;
            // 定型部分
